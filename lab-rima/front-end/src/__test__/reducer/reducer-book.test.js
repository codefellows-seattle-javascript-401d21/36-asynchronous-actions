  ...
  describe('language create action', () => {
    describe('valid input', () => {
      test('should create a new key(languageId) value(empty array) in app state', () => {
        ...
        expect(reducer({}, {type: 'LANGUAGE_CREATE', payload: mockLanguage})).toEqual({[mockLanguage.id]:[]});
      });
    });

    describe('invalid input', () => {
      test('should throw an error if language id is empty', () => {
        const mockLanguage = {name: 'new one', id: ''};
  ...
  describe('language delete action', () => {
    describe('valid input', () => {
      test('should return an updated state with language and all books that belong to that language removed', () => {
        const mockLanguage = {name: 'new one', id: '1'};
        const mockBook = {'1': [{id: '1', languageId: '1', title: 'test title', author: 'test author'}], '2': []};
  ...
  describe('book create action', () => {
    describe('valid input', () => {
      test('should return an app state with a new book', () => {
        const mockBook = {title: 'new book', author: 'test author', languageId: '1', id: '1'};
        const mockLanguage = {name: 'new one', id: '1'};
        let state = reducer({}, {type: 'LANGUAGE_CREATE', payload: mockLanguage});
        let expected = {'1': [mockBook]};
        expect(reducer(state, {type: 'BOOK_CREATE', payload: mockBook})).toEqual(expected);
      });
    });

    describe('invalid input', () => {
      test('should throw an error if language id is empty', () => {
        const mockBook = {name: 'new book', author: 'test', languageId: '', id: '1'};
      ...
      test('should throw an error if book id is empty', () => {
        const mockBookOne = {name: 'new book', author: 'tests', languageId: '1', id: ''};
        expect(() => reducer({}, {type: 'BOOK_CREATE', payload: mockBookOne})).toThrow('BOOK REDUCER: Book ID must be present.');
        const mockBookTwo = {name: 'new book', author: 'tests', languageId: '1', id: '   '};
  ...
  describe('book update action', () => {
    describe('book update action', () => {
      test('should return an app state with an updated book', () => {
        const mockBook = {title: 'new book', author: 'test', languageId: '1', id: '1'};
        const mockLanguage = {name: 'new one', id: '1'};
        let state = reducer({}, {type: 'LANGUAGE_CREATE', payload: mockLanguage});
        state = reducer(state, {type: 'BOOK_CREATE', payload: mockBook});
        const updatedMockBook = {title: 'updated book', author: 'test', languageId: '1', id: '1'};
        const expected = {'1': [updatedMockBook]};
        expect(reducer(state, {type: 'BOOK_UPDATE', payload: updatedMockBook})).toEqual(expected);
      });
    });

    describe('invalid input', () => {
      test('should throw an error if language id is empty', () => {
        const mockBook = {title: 'new book', author: 'test', languageId: '', id: '1'};
        expect(() => reducer({}, {type: 'BOOK_UPDATE', payload: mockBook})).toThrow('BOOK REDUCER: Book languageID must be present.');
      });
      test('should throw an error if book id is empty', () => {
        const mockBookOne = {title: 'new book', author: 'tests', languageId: '1', id: ''};
        expect(() => reducer({}, {type: 'BOOK_UPDATE', payload: mockBookOne})).toThrow('BOOK REDUCER: Book ID must be present.');
        const mockBookTwo = {name: 'new book', author: 'tesst', languageId: '1', id: '   '};
      ...
      test('should throw an error if title is empty', () => {
        const mockBook = {title: '', author: 'testss', languageId: '1', id: '1'};
        expect(() => reducer({}, {type: 'BOOK_UPDATE', payload: mockBook})).toThrow('BOOK REDUCER: Book title/author cannot be empty.');
      });
      test('should throw an error if author is empty', () => {
        const mockBook = {title: 'test', author: '', languageId: '1', id: '1'};
        expect(() => reducer({}, {type: 'BOOK_UPDATE', payload: mockBook})).toThrow('BOOK REDUCER: Book title/author cannot be empty.');
      });
    });
  });
  ...
  describe('book delete action', () => {
    describe('valid input', () => {
      test('should return an app state with a book removed', () => {
        const mockBook = {title: 'new book', author: '100', languageId: '1', id: '1'};
        const mockLanguage = {name: 'new one', id: '1'};
        let state = reducer({}, {type: 'LANGUAGE_CREATE', payload: mockLanguage});
        let expected = {'1': []};
    ...
    describe('invalid input', () => {
      test('should throw an error if language id is empty', () => {
        const mockBook = {title: 'new book', author: 'tetsts', languageId: '', id: '1'};
        expect(() => reducer({}, {type: 'BOOK_DELETE', payload: mockBook})).toThrow('BOOK REDUCER: Book languageID must be present.');
      });
      test('should throw an error if book id is empty', () => {
        const mockBookOne = {title: 'new book', author: 'test', languageId: '1', id: ''};
        expect(() => reducer({}, {type: 'BOOK_DELETE', payload: mockBookOne})).toThrow('BOOK REDUCER: Book ID must be present.');
        const mockBookTwo = {title: 'new book', author: '100', languageId: '1', id: '   '};
        expect(() => reducer({}, {type: 'BOOK_DELETE', payload: mockBookTwo})).toThrow('BOOK REDUCER: Book ID must be present.');
      });
      test('should throw an error if title is empty', () => {
        const mockBook = {title: '', author: '100', languageId: '1', id: '1'};
        expect(() => reducer({}, {type: 'BOOK_DELETE', payload: mockBook})).toThrow('BOOK REDUCER: Book title/author cannot be empty.');
      });
      test('should throw an error if author is empty', () => {
        const mockBook = {title: 'test', author: '', languageId: '1', id: '1'};
        expect(() => reducer({}, {type: 'BOOK_DELETE', payload: mockBook})).toThrow('BOOK REDUCER: Book title/author cannot be empty.');
